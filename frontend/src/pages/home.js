...
const Home = () => {
    ...
    if(search !== '' && layer[1].length !== 0) {
        searchData2.current = layer[1]
    }
    if(search === '' || layer[1].length === 0) {
        searchData2.current = layer[1]
    }
        ...
        <div id="home">
            ...
            <div id="searchdiv">
                <form id="searchform" onSubmit={searchFunc}>
                    <input type="text" placeholder="Search Here..." ref={refFoc} onChange={ch} onFocus={showFDiv} onBlur={unShowFDiv} value={search} autoFocus={MFormDiv === false}/>
                    ...
                    <div id="formSearchDiv" style={showFormDiv && searchData2.current[0].length !== 0 ? showFDivDes : unShowDivDes} onTouchStart={MFDiv} onTouchEnd={refFocus} onMouseOver={MFDiv} onMouseOut={unMFDiv} onClick={refFocus}>
                        {search.length !== 0 && searchData2.current[0].map(data =>
                            <div key={data._id}>
                                <p><Link to={`/pdfView/${data[6]}`} key={data[6]}>{data[0]}{data[1]}{data[2]}</Link></p>
                            </div>)
                        }
